"""Development-only runner for Stage C assists Candidate V1. This is NOT a promotion evaluation.

    python -m fpl.validate.dev_assists_candidate_v1            # full archive, development only
    python -m fpl.validate.dev_assists_candidate_v1 --season 2025-26

Candidate V1 (``xa_informed_trailing_player_assists_v1``) is a fixed closed-form estimator: the v1.0
trailing-player assist-rate baseline with its recent ASSISTS signal replaced by recent xA
(``expected_assists``) where xA is measured (per row, NULL filled by creativity rescaled to the
assist-rate scale) and the assist residual shrunk almost fully to the positional mean, falling back
to the exact v1.0 trailing-player baseline where xA is unmeasured. This runner reuses the Stage C
assists harness unchanged -- the same observed-gameweek folds, the same two frozen baselines on the
same eligible rows, the same proper-distribution metrics, slices, and counts -- but fits Candidate
V1
alongside those baselines on the *identical* eligible rows, scores it under the unchanged contract,
and prints a **DEVELOPMENT ONLY** report that **never** emits a promotion verdict.
...
time-of-use gap:
  ...
  * It records the Candidate V1 model-source fingerprint, so an edit to ``attacking_assists_v1.py``
    between capture and print is detected.
...
logger = logging.getLogger("fpl.validate.dev_assists_candidate_v1")

_DEVELOPMENT_BANNER = (
    "\n"
    "============================================================================\n"
    " DEVELOPMENT ONLY -- NOT A PROMOTION RESULT\n"
    " Candidate V1 (xa_informed_trailing_player_assists_v1). The historical archive is\n"
...
_CANDIDATE_SOURCE_REL = Path("src") / "fpl" / "models" / "attacking_assists_v1.py"
...
def load_contract_from_bytes(
    config_path: Path,
) -> tuple[Phase3StageCAssistsEvaluationConfig, str]:
...
# --------------------------------------------------------------------------------------
# Candidate V1 factory: fold-local, conforming to the assists harness protocol
# --------------------------------------------------------------------------------------


def candidate_factory(config: Phase3StageCAssistsEvaluationConfig) -> AssistsCandidateFactory:
...
def best_baseline_metric(
    result: AssistsHarnessResult, *, attr: str, higher_is_better: bool
) -> tuple[float, str]:
    ...
    Ties break on the frozen baseline order (``STAGE_C_ATTACKING_ASSISTS_BASELINE_ORDER``).
    """
...
def compute_development_diagnostics(
    result: AssistsHarnessResult, candidate: str, config: Phase3StageCAssistsEvaluationConfig
    ...
    best_brier, best_brier_name = best_baseline_metric(
        ...
        result,
        attr="mean_brier_at_least_one_goal",
        higher_is_better=False,
    )
    checks.append(
        _no_regression_check(
            "no_aggregate_brier_at_least_one_assist_regression",
            best_brier,
            cand.mean_brier_at_least_one_goal,
            allowed_regression=gate.maximum_brier_relative_regression_at_least_one_assist,
            bar=best_brier_name,
        )
    )
...
def format_development_report(
    ...
    *,
    ...
    # xA / fallback / cold-start path split (overall and by season) -- the headline diagnostic.
...
def build_reconciliation_record(
    ...
    *,
    provenance: Provenance,
    diagnostics: DevelopmentDiagnostics,
    schema: str = "stage_c_assists_candidate_v1_development/v1",
) -> dict[str, object]:
    """Build the complete result emitted for independent Stage C assists reconciliation."""
    return {
        "schema": schema,
...
def _standard_report(result: AssistsHarnessResult) -> str:
    """A concise score table mirroring the baseline harness output."""
    lines = [
        "=== Stage C Attacking Assists Candidate V1 Development Walk-Forward ===",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run Stage C assists Candidate V1 as a DEVELOPMENT-ONLY evaluation. "
    ...
    print("BEGIN_STAGE_C_ASSISTS_CANDIDATE_V1_RECONCILIATION_JSON")
    print(reconciliation_report)
    print("END_STAGE_C_ASSISTS_CANDIDATE_V1_RECONCILIATION_JSON")
