"""Development-only runner for Stage B Candidate V3. This is NOT a promotion evaluation.

    python -m fpl.validate.dev_minutes_candidate_v3            # full archive, development only
    python -m fpl.validate.dev_minutes_candidate_v3 --season 2025-26

Candidate V3 (``concentration_adaptive_shrinkage_player_minutes_v3``) is V2 with a
concentration-adaptive shrinkage strength; it reduces exactly to V2 at ``lambda = 0``. This runner
...
only supplies the V3 candidate factory, source path, banner, and reconciliation schema.

Pre-registered before any V3 evaluation. The same provenance fence closes every time-of-check /
time-of-use gap: it refuses a dirty worktree, fingerprints the exact parsed config bytes, the V3
...
NOTE: a V3 historical evaluation must run against a PRISTINE historical archive (rebuild via
``build_db``), because the working DB may carry live-registry rows so its whole-file fingerprint no
longer matches the V1/V2-era archive. Comparability is confirmed by the four baselines reproducing
their frozen values bit-for-bit, not by a whole-file hash. That one-shot run is the owner's
authorized action and is NOT invoked here.
"""
...
logger = logging.getLogger("fpl.validate.dev_minutes_candidate_v3")

_DEVELOPMENT_BANNER = (
    "\n"
    "============================================================================\n"
    " DEVELOPMENT ONLY -- NOT A PROMOTION RESULT\n"
    " Candidate V3 (concentration_adaptive_shrinkage_player_minutes_v3). V3 is V2 with a\n"
    " concentration-adaptive shrinkage strength (reduces to V2 at lambda=0). The historical\n"
...
_V3_SCHEMA = "stage_b_candidate_v3_development/v1"

# The Candidate V3 model source this runner scores. Fingerprinted so a mid-run edit is detected.
_CANDIDATE_SOURCE_REL = Path("src") / "fpl" / "models" / "minutes_v3.py"
...
def candidate_source_path(repo: Path) -> Path:
    """The Candidate V3 model source path whose fingerprint the runner records."""
    return repo / _CANDIDATE_SOURCE_REL


# --------------------------------------------------------------------------------------
# Candidate V3 adapter: a fold-local factory conforming to the harness protocol
# --------------------------------------------------------------------------------------


@dataclass(slots=True)
class _FittedCandidate:
    """Adapts Candidate V3's predict surface to the harness's uniform candidate shape."""

    name: str
    _model: ConcentrationAdaptiveShrinkagePlayerMinutesV3
...
def candidate_factory(config: Phase2EvaluationConfig) -> MinutesCandidateFactory:
    """Build the fold-local Candidate V3 factory (mirrors the V2 factory; imports V3 lazily)."""
    from fpl.models.minutes_v3 import ConcentrationAdaptiveShrinkagePlayerMinutesV3

    def factory(history: tuple[HistoryRow, ...], as_of: datetime) -> _FittedCandidate:
        model = ConcentrationAdaptiveShrinkagePlayerMinutesV3(config=config).fit(
            history, as_of=as_of
        )
...
# --------------------------------------------------------------------------------------
# Orchestration (mirrors dev_minutes_candidate_v2.main; reuses every shared helper)
# --------------------------------------------------------------------------------------
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run Stage B Candidate V3 as a DEVELOPMENT-ONLY evaluation. Not a promotion evaluation."
        )
    )
    ...
    candidate_name = contract.stage_b_candidate_v3.name
    # Refuse to score a dirty worktree so the provenance is truthful.
    require_clean_worktree(repo, candidate="Candidate V3")
    ...
    reconciliation = build_reconciliation_record(
        result, contract, provenance=provenance, diagnostics=diagnostics, schema=_V3_SCHEMA
    )
    ...
    print("BEGIN_STAGE_B_CANDIDATE_V3_RECONCILIATION_JSON")
    print(format_reconciliation_record(reconciliation))
    print("END_STAGE_B_CANDIDATE_V3_RECONCILIATION_JSON")
    return 0
