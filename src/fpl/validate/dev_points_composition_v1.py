"""Development-only runner for Stage D v1 points composition. This is NOT a promotion evaluation.

    python -m fpl.validate.dev_points_composition_v1          # full archive, development only
    python -m fpl.validate.dev_points_composition_v1 --season 2025-26

Stage D v1 composes the fitted Stage A/B/C component distributions into a per-player-fixture points
...
rechecks nothing moved during the run and emits a full reconciliation record and a report.

**Every number it prints is DEVELOPMENT-ONLY and EXPLORATORY -- not a promotion verdict.** There is
no pre-registered Stage D promotion contract; this is the first end-to-end "is it any good" reading,
under the same unversioned historical proxies (target roster, first-kickoff cutoff) every stage
carries, and with the documented Stage-D-v1 limitations (component independence / no team-coupling,
bonus excluded, defensive contribution = 0 historically). See
...
logger = logging.getLogger("fpl.validate.dev_points_composition_v1")

_DEVELOPMENT_BANNER = (
    "\n"
    "============================================================================\n"
    " DEVELOPMENT ONLY -- NOT A PROMOTION RESULT\n"
    " Stage D v1 points composition. The historical archive is development evidence\n"
    " (unversioned target-roster / first-kickoff proxies) and every component is itself a\n"
    " development-stage estimator; this is the first end-to-end reading, not a fresh holdout.\n"
    " Stage-D-v1 limitations: components composed independently (no team-coupling / goal\n"
    " conservation), bonus excluded from both prediction and label, defensive contribution = 0\n"
    " historically. No number below is a promotion verdict.\n"
    "============================================================================\n"
)
...
def require_clean_worktree(repo: Path) -> None:
    ...
    if porcelain:
        raise SystemExit(
            "Stage D v1 refuses to run on a dirty worktree. The recorded commit SHA must name the "
...
def build_reconciliation_record(
    ...
    return {
        "schema": "stage_d_points_composition_v1_development/v1",
        ...
        "limitations": {
            ...
            "defensive_contribution": "held at 0 historically (data exists only for 2025-26)",
            "unmodelled_components": (
                "saves, penalties, own goals, cards not drawn (label still includes them)"
            ),
            "negative_points": "folded into the 0 bin to match the count-metric clamp",
        },
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run Stage D v1 points composition as a DEVELOPMENT-ONLY evaluation. Not a promotion "
            "evaluation."
        )
    )
    ...
    try:
        ...
        sys.stderr.write(
            f"Stage D v1 result is INVALID / UNPUBLISHABLE and will not be printed: {exc}\n"
        )
        return 1
    ...
    print("BEGIN_STAGE_D_POINTS_COMPOSITION_V1_RECONCILIATION_JSON")
    print(reconciliation_report)
    print("END_STAGE_D_POINTS_COMPOSITION_V1_RECONCILIATION_JSON")
