  ...
  return <article aria-label={`${match.home.team.team_name} v ${match.away.team.team_name}`} className="rounded-2xl border bg-card p-4 shadow-sm sm:p-5">
    ...
    <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3">
      {[match.home, match.away].map((side, index) => <div key={side.team.team_code} className={index === 0 ? "col-start-1 row-start-1 min-w-0" : "col-start-3 row-start-1 min-w-0 text-right"}>
        <div className={`mb-2 flex items-center gap-2 ${index ? "justify-end" : ""}`}><TeamBadge teamCode={side.team.team_code} shortName={side.team.short_name} size="md" /><span className="text-xs font-semibold text-muted-foreground">{index ? "AWAY" : "HOME"}</span></div>
        <h3 className="min-h-10 text-sm font-semibold leading-5 sm:text-base">{side.team.team_name}</h3>
        <p className="mt-3 text-3xl font-semibold tabular-nums tracking-tight">{goal(side.forecast.lambda_for)}</p>
      </div>)}
      <span className="col-start-2 row-start-1 text-xs font-medium text-muted-foreground">vs</span>
    </div>
    <p className="mt-2 text-center text-xs text-muted-foreground">Expected goals · not an exact-score pick</p>
    <div className="mt-4 flex items-center justify-between gap-2 rounded-lg bg-emerald-50 px-3 py-2.5 text-sm dark:bg-emerald-950/30">
      <span className="font-semibold tabular-nums">{chance(match.home.forecast.probability_clean_sheet)}</span><span className="text-xs text-emerald-900 dark:text-emerald-200">Clean-sheet chance</span><span className="font-semibold tabular-nums">{chance(match.away.forecast.probability_clean_sheet)}</span>
    </div>
  </article>;
}
...
export function GwAnalysisPage() {
  ...
  return <div className="mx-auto w-full max-w-7xl space-y-6 p-4 pb-12 lg:p-6">
    ...
    {state.status === "loading" ? <p role="status" className="rounded-xl border p-6 text-sm text-muted-foreground">Loading the matchweek outlook…</p> : state.status === "unavailable" ? <div role="status" className="rounded-xl border border-dashed p-6"><h2 className="font-semibold">This matchweek is not available yet</h2><p className="mt-2 text-sm text-muted-foreground">The forecast could not be loaded. Please try again later.</p></div> : <>
      ...
      <section aria-labelledby="fixtures-heading"><div className="mb-4 flex flex-wrap items-baseline justify-between gap-2"><h2 id="fixtures-heading" className="text-xl font-semibold tracking-tight">The fixture outlook</h2><a href="#fixtures" className="inline-flex min-h-11 items-center gap-1 text-sm font-medium underline-offset-4 hover:underline">Fixture calendar<ArrowUpRight className="size-4" aria-hidden="true" /></a></div><p className="mb-4 text-sm text-muted-foreground">Exact-score analysis has not been published for this gameweek. These goal averages and clean-sheet chances provide the current outlook.</p><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{matches.map(match => <MatchOutlook key={match.fixture} match={match} />)}</div>{!matches.length && <p className="rounded-xl border border-dashed p-6 text-sm text-muted-foreground">No complete match forecasts are available for this gameweek.</p>}</section>
