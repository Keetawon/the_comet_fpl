...
function NewsCard({ story, language, demo, selected, compact }: { story: NewsStory; language: NewsLanguage; demo: boolean; selected: boolean; compact: boolean }) {
  ...
  return <article id={`news-${story.id}`} className={`min-w-0 scroll-mt-4 ${compact ? "py-4 first:pt-0 last:pb-0" : "rounded-2xl border bg-card p-5 sm:p-6"} ${selected ? "ring-2 ring-sky-500" : ""}`} aria-labelledby={`title-${story.id}`}>
...
export function NewsFeed({ compact = false }: { compact?: boolean }) {
  ...
  return <section className={compact ? "min-w-0 rounded-2xl border bg-card p-5 sm:p-6" : "min-w-0"} aria-label={t.title}>
    <header className={`flex flex-wrap items-start justify-between gap-4 ${compact ? "" : "rounded-2xl border bg-muted/25 p-5 sm:p-7"}`}>
      <div className="max-w-2xl">
        {!compact && <p className="mb-3 flex items-center gap-2 text-xs font-bold tracking-[0.14em] text-amber-800 dark:text-amber-300"><Newspaper className="size-4" aria-hidden="true" />{t.eyebrow}</p>}
    ...
    {!compact && feed && feed.stories.length > 0 && <div className="mt-6 space-y-4">
      <div className="flex flex-col gap-2 sm:flex-row"><label className="relative min-w-0 flex-1"><Search className="pointer-events-none absolute top-3.5 left-3 size-4 text-muted-foreground" aria-hidden="true" /><span className="sr-only">{t.search}</span><input value={query} onChange={event => setQuery(event.target.value)} placeholder={t.search} className={`${fieldClass} w-full pl-9`} /></label><label><span className="sr-only">{t.club}</span><select value={team} onChange={event => setTeam(event.target.value)} className={`${fieldClass} w-full sm:max-w-56`}><option value="">{t.allTeams}</option>{clubs.map(([code, name]) => <option value={code} key={code}>{name}</option>)}</select></label><button type="button" className={fieldClass} onClick={reset}>{t.reset}</button></div>
      <div className="flex flex-wrap gap-2" role="group" aria-label={language === "th" ? "ประเภทข่าว" : "News category"}>{(["", ...Object.keys(categoryNames)] as (NewsCategory | "")[]).map(key => <button type="button" key={key} aria-pressed={category === key} onClick={() => setCategory(key)} className={`inline-flex min-h-11 items-center gap-2 rounded-full border px-3.5 py-2 text-sm font-medium focus-visible:outline-2 ${category === key ? "border-foreground bg-foreground text-background" : "bg-card hover:bg-muted"}`}>
    ...
    <div className={compact ? "mt-5" : "mt-5 grid gap-6 xl:grid-cols-[minmax(0,1fr)_17rem]"}>
      ...
      {!compact && <aside className="space-y-4 self-start">
        {clubs.length > 0 && <div className="rounded-2xl border bg-card p-5"><h2 className="text-sm font-semibold">{t.clubs}</h2><div className="mt-3 flex flex-wrap gap-2">{clubs.map(([code, name]) => <button type="button" key={code} aria-pressed={team === String(code)} onClick={() => setTeam(team === String(code) ? "" : String(code))} className={`min-h-11 rounded-lg border px-3 py-2 text-xs font-medium focus-visible:outline-2 ${team === String(code) ? "bg-foreground text-background" : "hover:bg-muted"}`}>{name}</button>)}</div></div>}
