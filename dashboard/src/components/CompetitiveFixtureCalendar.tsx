...
import { calendarColumns, calendarFixtures, calendarRange, dailyCalendarColumns, listedClubGaps, MAX_DAILY_DAYS, leagueSlot, visibleCalendar, type CalendarFixture } from "@/lib/competitiveCalendar";
...
export function CompetitiveFixtureCalendar({ teams, schedule, fromGw, toGw }: {
  ...
  const columns = view === "daily" ? (invalidRange ? [] : dailyCalendarColumns(visible.rows, from, to, breaks))
  ...
  const exportCsv = () => {
    ...
    const lines = [["Team", ...columns.map((c) => `${c.heading} · ${view === "daily" ? c.from : c.label}`)], ...clubs.map((t) => [t.team_name, ...columns.map((c) => {
  ...
  return <section className="space-y-3" aria-label="All-competition fixture calendar">
    <div className="rounded-lg border bg-card p-3 space-y-3">
      ...
      <div className="flex flex-wrap gap-3 items-end text-sm">
        ...
        <Button variant="ghost" size="sm" onClick={() => { setRange(null); setHorizon(toGw - fromGw + 1); setSelected([]); setColour("opponent"); setView("weekly"); }}>Reset calendar</Button>
      </div>
          ...
          <TooltipProvider delayDuration={200}>
            <table aria-label={view === "daily" ? "All competitions by day" : "All competitions by period"} className="table-fixed border-collapse text-xs tabular-nums" style={{width: 152 + columns.length * columnWidth}}>
              ...
              <thead className="sticky top-0 z-20 bg-muted"><tr><th scope="col" className="sticky left-0 z-30 border-b border-r bg-muted px-3 py-2 text-left">Club</th>
                {columns.map((c) => <th key={c.key} scope="col" title={`${c.from} – ${c.to} · UK dates${c.internationalBreak ? " · International break" : ""}`} className={`border-b border-r px-1 py-2 text-center font-medium ${c.internationalBreak ? internationalClass : view === "weekly" && c.heading !== "Weekend" ? "bg-sky-50 text-sky-950 dark:bg-sky-950 dark:text-sky-100" : ""}`}><span className="block text-[10px] text-muted-foreground">{c.heading}</span><span className="block text-[11px] leading-tight">{c.label}</span>{view === "daily" && c.internationalBreak && <span className="text-[9px]">INT</span>}</th>)}</tr></thead>
    ...
    <details className="rounded border p-3 text-xs text-muted-foreground"><summary className="cursor-pointer">Sources & schedule coverage</summary><p className="mt-2">FPL schedule exported {stamp(schedule.export_created_at)} UK. This calendar is separate from immutable prediction vintages and does not change xP, optimizer inputs or player minutes.</p>
      <p className="mt-2">International breaks: {season === "2026-27" ? <><a href={INTERNATIONAL_BREAK_SOURCE.url} target="_blank" rel="noreferrer" className="underline">{INTERNATIONAL_BREAK_SOURCE.name}</a>, published {INTERNATIONAL_BREAK_SOURCE.publishedOn}; verified {stamp(INTERNATIONAL_BREAK_SOURCE.verifiedAt)} UK. Yellow covers each published window: one column in Weekly, each included date in Daily. Dates are not inferred from missing fixtures; no individual national-team participation is captured here.</> : "No verified international calendar retained for this season."}</p>
