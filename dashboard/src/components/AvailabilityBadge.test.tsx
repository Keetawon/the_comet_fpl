...
describe("reported availability badge", () => {
  ...
  it.each(["i", "s"])("makes official %s status dark red, without inventing a missing percentage", status => {
    const record: PlayerRecord = { ...player, current_availability: {
      ...player.current_availability!, status, chance_of_playing_next_round: 0,
    } };
    ...
    rerender(<AvailabilityBadge player={{ ...record, current_availability: {
      ...record.current_availability!, chance_of_playing_next_round: null,
    } }} />);
  ...
  it("does not turn an available status with no reported chance into 100%", () => {
    render(<AvailabilityBadge player={{ ...player, current_availability: {
      ...player.current_availability!, status: "a", chance_of_playing_next_round: null,
    } }} />);
    expect(screen.getByText("available")).toHaveClass("text-muted-foreground");
    expect(screen.queryByText(/%/)).not.toBeInTheDocument();
  });
});
