...
export function AvailabilityBadge({ player, details = false }: { player: PlayerRecord; details?: boolean }) {
  ...
  const label = status == null ? "Unknown" : availabilityLabel(status);
  ...
  const title = [source, chance == null ? null : `Reported next-round chance: ${chance}%`, current?.news, "Reported availability only; published xP is unchanged."].filter(Boolean).join("\n");
  ...
  return (
    <span className="inline-flex flex-col text-xs" title={title}>
      <span className={`self-start whitespace-nowrap font-medium ${tone}`}>
        {label}
      </span>
