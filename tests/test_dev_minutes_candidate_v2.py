"""Deterministic OFFLINE tests for the Stage B Candidate V2 development runner.

The V2 runner reuses the V1 runner's provenance machinery, the shared best-per-metric gate, and the
harness -- all already tested via test_dev_minutes_candidate_v1.py. These tests pin only the V2
wiring: the candidate name, the V2 model-source path it fingerprints, and the V2 reconciliation
...
import fpl.validate.dev_minutes_candidate_v2 as dev2
from fpl.config import config_dir, repo_root
from fpl.validate.dev_minutes_candidate_v2 import candidate_source_path
...
CANDIDATE = "recency_weighted_trailing_player_minutes_v2"
BASELINES = (
...
def _sandbox(tmp_path: Path) -> tuple[Path, Path, Path]:
    """A clean tmp repo with a committed V2 candidate source, plus sibling config and db files."""
    repo = _init_repo(tmp_path / "repo")
    source_dir = repo / "src" / "fpl" / "models"
    source_dir.mkdir(parents=True)
    (source_dir / "minutes_v2.py").write_bytes(
        (repo_root() / "src" / "fpl" / "models" / "minutes_v2.py").read_bytes()
    )
    subprocess.run(["git", "add", "-A"], cwd=repo, check=True)
    subprocess.run(["git", "commit", "-q", "-m", "v2 source"], cwd=repo, check=True)
    ...
    assert candidate_source_path(repo) == source_dir / "minutes_v2.py"
    return repo, cfg_dir, db


def test_candidate_source_path_targets_minutes_v2() -> None:
    assert (
        candidate_source_path(repo_root())
        == repo_root() / "src" / "fpl" / "models" / "minutes_v2.py"
    )


def test_main_success_path_prints_v2_schema_and_uses_read_only(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    repo, cfg_dir, db = _sandbox(tmp_path)
    monkeypatch.setattr(dev2, "repo_root", lambda: repo)
    monkeypatch.setattr(dev2, "config_dir", lambda: cfg_dir)
    events: list[object] = []
    ...
    real_verify = dev2.verify_snapshot
    ...
    monkeypatch.setattr(dev2, "run_minutes_harness", fake_run)
    monkeypatch.setattr(dev2, "verify_snapshot", verify_wrapper)

    rc = dev2.main(["--db", str(db)])

    assert rc == 0
    out, _err = capsys.readouterr()
    # The V2 reconciliation schema and candidate name are emitted, not V1's.
    assert "BEGIN_STAGE_B_CANDIDATE_V2_RECONCILIATION_JSON" in out
    assert '"schema": "stage_b_candidate_v2_development/v1"' in out
    assert CANDIDATE in out
    assert "DEVELOPMENT ONLY" in out
    assert "DEVELOPMENT DIAGNOSTIC ONLY" in out
    # The starter-ranking gate (shared) is reported and the V2 candidate (0.72) clears it.
    ...
    # A V2 candidate factory was supplied to the harness.
    assert callable(captured["candidate_factory"])
    # No V1 schema leaks into the V2 output.
...
def test_main_performs_no_archive_or_network_access(
    ...
    monkeypatch.setattr(dev1, "connect", fake_connect)
    monkeypatch.setattr(dev2, "run_minutes_harness", lambda *a, **k: _canned_result())
