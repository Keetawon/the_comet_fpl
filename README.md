...
append-only forecast/outcome ledgers, BI semantic contract version 4, dashboard schema version 6,
...
development-only. Dashboard read-model schema v6 retains the backend-convolved cumulative player
...
raw PMFs stay out of the bulk payload. It also publishes finalized, same-season player-fixture
actuals for the Players page's independent `Actual GWs` range; prior-season fallback and partial
official gameweeks are excluded, while double-gameweek legs remain distinct. The forecast values are raw
...
the cumulative player endpoint file introduced in schema v4 and retained in schema v6. Schema v6
also carries only finalized same-season player actual fixtures for the independent Players-page
actual range. Dashboards
