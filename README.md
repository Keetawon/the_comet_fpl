...
append-only forecast/outcome ledgers, BI semantic contract version 4, dashboard schema version 7,
...
development-only. Dashboard read-model schema v7 retains the backend-convolved cumulative player
...
raw PMFs stay out of the bulk payload. It also publishes normalized finalized player-fixture
...
actual pages, plus Plan Builder and a browser-only Squad Draft sandbox. Analytics charts use
concise edge-aware tooltips and per-view reading notes while retaining full provenance in their
accessible labels and tables. Player analytics excludes only forecast-marked cold starts by
default, with an explicit include control; this changes reporting eligibility, not xP. The legacy
...
the cumulative player endpoint file introduced in schema v4 and retained in schema v7. Schema v7
...
explicit prior/current-season Players-page actual ranges. Dashboards
...
Builder, and Squad Draft. Every route has an immediate network-free deterministic summary. Exactly
seven public analytical routes (Summary, Fixture matrix, Players, both analytics pages, and both
prediction-vs-actual pages) may offer **Explain with AI** through the protected local server; the
four decision/private routes remain deterministic-only. Provider use is explicit opt-in and
evidence-bound, and the hosted static build makes no provider call. See
`docs/dashboard-deep-analytics.md`, `docs/prediction-vs-actual-dashboard.md`, and
